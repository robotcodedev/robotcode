...
class RobotDefinitionProtocolPart(GenericJsonRPCProtocolPart["RobotLanguageServerProtocol"]):
    ...
    def __init__(self, parent: "RobotLanguageServerProtocol") -> None:
        ...
        parent.definitions.collect_definitions.add(self.collect_definitions)
            ...
            if isinstance(result_node, KeywordCall):
                ...
                if node.keyword:
                    ...
                    if position.is_in_range(range_from_token(keyword_token)):
                        ...
                        if (
                            keyword_doc is not None
                            and keyword_doc.parent is not None
                            and keyword_doc.parent.source is not None
                        ):
                            return [
                                LocationLink(
                                    origin_selection_range=range_from_token_or_node(node, keyword_token),
                                    target_uri=str(Uri.from_path_str(keyword_doc.parent.source)),
