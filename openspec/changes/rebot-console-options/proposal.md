...
- `robotcode rebot` passes them to `rebot` on Robot Framework ≥ 7.5. On older versions they are dropped with a verbose note instead of breaking `rebot` (`option --console not a unique prefix`).
...
- `robot-toml-option-coverage`: adds the requirement "Rebot console options" to the capability introduced by `support-rf75` (not yet archived): `[rebot] console`/`quiet` are accepted, forwarded on RF ≥ 7.5, dropped with a note on older versions, and independent of the top-level `console`/`quiet`.
...
- `packages/robot/src/robotcode/robot/config/model.py`: regenerated region (`RebotOptions.console`, `RebotOptions.quiet`); `BaseOptions.add_options` gains an opt-in `exclude` parameter.
- `packages/runner/src/robotcode/runner/cli/rebot.py`: exclusion of top-level `console`/`quiet`, RF < 7.5 guard, dry-run message.
