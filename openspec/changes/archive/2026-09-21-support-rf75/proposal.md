...
Robot Framework 7.5 was released on 2026-09-14 and is not part of RobotCode's test matrix. Running RobotCode against it silently loses information and rejects valid input: keyword tags declared in documentation (`Tags:` lines, including `robot:private`) disappear, the standard libraries — now documented in Markdown with Google-style docstrings — render their type documentation as raw Robot markup, a library using a recursive Python 3.12 type alias loses all of its type documentation, the "show documentation" web view needs the optional `markdown` package for every Markdown-documented standard library, `robot.toml` refuses the custom console loggers RF 7.5 accepts, `${TEST_METADATA}` is reported as an unknown variable, and `Resource    file.md` — valid in RF 7.5 — is flagged as an invalid extension. On top of that the generator that produces the `robot.toml` model is stale, so the model cannot be regenerated for the new options without first repairing it.
...
- **Library and resource documentation on RF ≥ 7.5** (behaviour restored to what RF ≤ 7.4 gave, without mutating Robot Framework's objects):
  ...
  - Libdoc HTML generation (the documentation web view, `robotcode libdoc`) for Markdown-documented libraries needs the `markdown` package, which is optional in Robot Framework. RobotCode treats it like `docutils` for reStructuredText: it is neither a dependency nor bundled, users install it when they need it, and without it the web view shows Robot Framework's message that the module is missing. For RobotCode's own tests it is a development dependency in the hatch configuration.
...
- **Documentation**: a section in the getting-started requirements about the optional packages `docutils` and `markdown` (what they are needed for, that users install them themselves); regenerated `config.md`; regeneration procedure for the `robot.toml` model in `CONTRIBUTING.md`; a note that RF < 7.5 cannot read an RF 7.5 `output.xml` containing test metadata (affects `robotcode results`); a note in the variable-not-found patch that RF 7.5 fixed the finder-miss path but the patch still pays off.
...
- `hatch.toml`: `markdown` as a development dependency (not a package dependency, not bundled).
...
- Upstream: nothing is reported to Robot Framework for now; the block-relative line numbers in Markdown/reST code blocks are tracked in RobotCode issue #651 (embedded suites are not supported yet), hiding private keywords in completion in #652, see design.md Open Questions.
- `AGENTS.md`, `CONTRIBUTING.md`, `.github/copilot-instructions.md`, `.github/instructions/python-development.instructions.md`, `docs/03_reference/analyzing-results.md`, `docs/02_get_started/index.md`.
