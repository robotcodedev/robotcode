...
- **Text output**: `discover all/tests/tasks` and `results show` gain `--metadata/--no-metadata` (default off on all four commands; `--tags` defaults to on for `discover all` only) printing `- _Metadata:_ name: value, …`; `results log` prints metadata bullets under a test header whenever present.
- **VS Code test explorer**: test items show their metadata compactly in the item description (`test  - Issue: 4409, Author: febb0e`, truncated), and the typed execution attributes declare `metadata`; metadata-only edits trigger a refresh.
...
- `test-metadata`: How test/task-level metadata from Robot Framework ≥ 7.5 is represented and shown across RobotCode — discover and results JSON and text output, search, the VS Code test explorer, the IntelliJ discover model and the test hover — and how files with test metadata are handled on older Robot Framework versions.
...
- `packages/runner/src/robotcode/runner/cli/_search.py`: import-time `test_metadata()` helper, search matching.
- `packages/runner/src/robotcode/runner/cli/discover/_models.py`, `discover.py`, `_render.py`: `metadata` field, collector, `--metadata` flag, text rendering.
- `packages/runner/src/robotcode/runner/cli/results/_models.py`, `results.py`, `_render.py`: `metadata` on `TestResultItem` and `LogTest`, `--metadata` flag for `show`, log rendering, reader hint.
...
- `vscode-client/extension/testcontrollermanager.ts`: `RobotTestItem.metadata`, `RobotExecutionAttributes.metadata`, equality/snapshot, description.
