...
### Requirement: Libdoc HTML works for Markdown-documented libraries

Generating Libdoc HTML through RobotCode (the documentation web view and `robotcode libdoc`) SHALL work for libraries documented in Markdown when RobotCode is installed with its declared dependencies or from the bundled editor extensions.

#### Scenario: Documentation web view for a standard library
- **WHEN** the HTML documentation of `Collections` is requested on RF 7.5 in an environment installed from RobotCode's packages or the bundled VS Code extension
