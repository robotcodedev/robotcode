...
The model-based semantic-tokens path (Tier 1 of the SemanticModel migration)
produces output identical to the legacy AST-based path for every construct in
the LSP test data, verified by a non-vacuous dual-protocol parity suite. This
parity is the foundation of the migration's verification story: if semantic
tokens match, the model is correct for all other consumers.
